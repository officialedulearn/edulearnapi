...
async function cleanupOrphanedUsers() {
  ...
  try {
    ...
    if (orphanedUsers.length === 0) {
      console.log(
        '✅ No orphaned users found! Database and Supabase Auth are in sync.\n',
      );
      return;
    }
    ...
    orphanedUsers.forEach((user, index) => {
      console.log(
        `${index + 1}. ${user.email} (ID: ${user.id}) - Created: ${user.created_at}`,
      );
    });
      ...
      async (answer: string) => {
        ...
        for (const orphanedUser of orphanedUsers) {
          try {
            ...
            if (error) {
              console.error(
                `❌ Failed to delete ${orphanedUser.email}:`,
                error.message,
              );
              failCount++;
            } else {
        ...
        if (successCount > 0) {
          ...
          console.log(
            `   Supabase Auth users: ${allAuthUsers.length - successCount} (estimated)`,
          );
          console.log(
            `   Difference: ${Math.abs(dbUsers.length - (allAuthUsers.length - successCount))}\n`,
          );
        }
